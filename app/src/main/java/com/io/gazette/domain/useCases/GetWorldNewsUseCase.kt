...
class GetWorldNewsUseCase(private val nytRepository: NytRepository) {

    operator fun invoke(): Flow<RequestResult<List<NewsItem>>> = flow {
        try {
            emit(Loading)

            val worldNewsList = nytRepository.getWorldNews().results?.map {
                it.toDomainNewsItem()
            }

            emit(Success(worldNewsList.orEmpty()))
