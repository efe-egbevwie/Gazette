...
class GetSportsNewsUseCase(private val nytRepository: NytRepository) {
    ...
    operator fun invoke(): Flow<RequestResult<List<NewsItem>>> = flow {
        try {
            ...
            val sportsNewsList =
                nytRepository.getSportsNews().results?.map { it.toDomainNewsItem() }

            emit(Success(sportsNewsList.orEmpty()))
