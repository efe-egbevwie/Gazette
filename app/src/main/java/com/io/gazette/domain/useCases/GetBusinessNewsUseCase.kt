...
class GetBusinessNewsUseCase(private val nytRepository: NytRepository) {

    operator fun invoke(): Flow<RequestResult<List<NewsItem>>> = flow {
        try {
            emit(Loading)
            val businessNewsList = nytRepository.getBusinessNews().results?.map {
                it.toDomainNewsItem()
            }
            emit(Success(businessNewsList.orEmpty()))
